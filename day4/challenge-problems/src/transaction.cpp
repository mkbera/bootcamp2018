...
bool txn_t::validate() 
{
    ...
    // check lengths of addresses.
    if (source_addr.size() != SHA256_DIGEST_LENGTH) { return false; }
    if (dest_addr.size() != SHA256_DIGEST_LENGTH) { return false; }
    if (change_addr.size() != SHA256_DIGEST_LENGTH) { return false; }

    // make sure hash(pubkey) == src_addr
    uint8_t pkeyHash[SHA256_DIGEST_LENGTH];
    sha256(public_key.data(), public_key.size(), pkeyHash);
    for (unsigned i=0; i != SHA256_DIGEST_LENGTH; i++) {
        if(pkeyHash[i] != source_addr[i]) {
            return false;
        }
    }

    // check size.
    if (tx_hash.size() != SHA256_DIGEST_LENGTH) {
        return false;
    }
    ...
    return (valid = pubkey.verify(hash, SHA256_DIGEST_LENGTH, 
