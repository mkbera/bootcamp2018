...
{
    hash_result_t computed_blk_hash;
    ...
    for (auto tx : transactions) {
        ...
        if (!tx->balance_available(balances)) {
            std::cout << "balance not available." << std::endl;
            valid = false;
            return false;
        }
    ...
    // compute hash.
    compute_block_hash(computed_blk_hash);
    // compare the tx_hash with these bytes
    valid = true;
    for (unsigned i = 0; i != SHA256_DIGEST_LENGTH; i++) {
        valid = valid && (computed_blk_hash[i] == blk_hash[i]);
        if (!valid) { 
